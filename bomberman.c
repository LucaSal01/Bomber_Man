...
int *coordinates(char *str)
{
    int *pos = malloc(sizeof(int));

    char *context;
    size_t t;
    char counter[sizeof(int)];
    pos[0] = atoi(strtok_s(str, "", &context));

    _itoa_s(pos[0], counter, t, 10);
    pos[1] = atoi(strtok_s(str + strlen(counter), "", &context));
    return pos;
}
int set_nb(int s)
{
